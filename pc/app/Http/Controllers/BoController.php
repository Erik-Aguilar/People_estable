        ...
        switch ($campa) {
            case 'TM Prepago':
                ...
                $reg = TmPreBo::where(['dn' => $request->dn])->update(
                        ...
                        #'obs' => date('d/m/Y H:m:s')." - ".$request->estatus."\n".$request->observaciones."\n"
                ]);
#xxx
    ...
    public function InicioCM(){
        $menu=$this->menu();
        return view('bo.jefebo.InicioCM', compact('menu'));
    }

    public function subeCM(Request $request){
        ...
        $query = sprintf("LOAD DATA local INFILE '%s' INTO TABLE pc_mov_reportes.pre_cm FIELDS TERMINATED BY '|' lines terminated by '\\n' ignore 1 lines", addslashes($archivo_venta));
        
        DB::connection()->getpdo()->exec($query);

        DB::connection()->getpdo()->exec("call pc_mov_reportes.stg_cm()");

        #dd($new);
        return redirect('/subeCM');
    }
